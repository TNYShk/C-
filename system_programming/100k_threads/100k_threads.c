...
void Ex3(void)
{
    ...
    for (;i < TH_ARR_SIZE; ++i)
    {
        while(0 != pthread_create(&thread_id[i], NULL, ThreadFunc, NULL ));
...
void Ex4(void)
{
    ...
    {
        while(SUCCESS != pthread_create(&thread_id[i], NULL, &ThreadDivFunc, (void *)num));
        num += range;
    }
