...
void TestOne()
{
	...
	printf("inserted After first, 2rd value is %f\n",*(float *)float_ptr);
...
void TestTwo()
{
	...
	printf("inserted 1st value to twinky, value is %d\n",*(int *)int_ptr);
	
	k_second = SListInsertBefore(k_start, &t);
	int_kptr = SListGetData(k_second);
	printf("inserted 1st value to kika, = value is %d\n",*(int *)int_kptr);
	printf("\n\t********************Asserting Inserted******************\n");
	assert(second != k_second);
	printf("\n\t********************adding nodes******************\n");
	third = SListInsertBefore(second, &ttt);
	int_ptr = SListGetData(third);
	printf("inserted 2st value to Q1, Before first = value is %d\n",*(int *)int_ptr);

	k_end = SListInsertAfter(k_second, &tt);
	int_kptr = SListGetData(k_end);
	printf("inserted 2st value to Q2, After first = value is %d\n",*(int *)int_kptr);

	printf("num of elements in Q1 is %ld\n",SListCount(twinky));
	printf("num of elements in Q2 is %ld\n",SListCount(kika));
	

	printf("\n\t********************Testing Add Func******************\n");
	int_kptr = SListGetData(k_start);
	printf("Pre add Q2 1st node value is %d\n",*(int *)int_kptr);

	int_kptr = SListGetData(k_end);
	printf("Pre add Q2 2st node value is %d\n",*(int *)int_kptr);
	SListForEach(SListBegin(kika), SListEnd(kika),AddSome, &t);

	int_kptr = SListGetData(k_start);
	printf("POST Add Func to Q2, 1st node value is %d\n",*(int *)int_kptr);

	int_kptr = SListGetData(k_end);
	printf("POST Add Func to Q2, 2st node value is %d\n",*(int *)int_kptr);

	printf("\n\t********************Append Q1 into Q2******************\n");

	SlistAppend(kika, twinky);
	printf("\npost append number of nodes Q2: %lu\n",SListCount(kika));
	printf("\npost append number of nodes Q1: %lu\n",SListCount(twinky));
	...
	printf("inserted node to Q1, its value  %d\n",*(int *)int_ptr);
	printf("\nPost Append and insertion into Q1 count is: %lu\n",SListCount(twinky));
	assert(SListCount(kika) != SListCount(twinky));
	printf("\nPost Append number of nodes Q2: %lu\n",SListCount(kika));
	printf("\n\t*************************Adding nodes to Q2***********************\n");
	k_append = SListInsertBefore(k_end, &t);
	int_kptr = SListGetData(k_append);
	printf("POST Append, adding node to Q2, its value is %d\n",*(int *)int_kptr);
	printf("\nnumber of nodes Q2, post add: %lu\n",SListCount(kika));
	printf("\nnumber of nodes in Q1:  %lu\n",SListCount(twinky));
	SListDestroy(twinky);
	SListDestroy(kika);
